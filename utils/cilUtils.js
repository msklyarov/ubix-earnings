...
class CilUtils {
    constructor(options) {
        const {rpcAddress, rpcPort, rpcUser, rpcPass, nFeeDeploy, nFeeInvoke, nFeePerInputOutput, privateKey, apiUrl} =
            options;
    ...
    async createTxWithFunds({
        ...
        if (change > 0) {
            ...
            if (change > 0) tx.addReceiver(change, Buffer.from(strFromAddress || this._kpFunds.address, 'hex'));
        }

        // for single PK scenario it's allowed to use single claimProof in txSignature
        if (tx.inputs.length > 1) {
            tx.signForContract(strFromPk || this._kpFunds.privateKey);
        } else {
            for (let i in tx.inputs) {
                tx.claim(parseInt(i), strFromPk || this._kpFunds.privateKey);
            }
        }
    ...
    async isTxDoneExplorer(strTxHash, bContractCall) {
        const hasInternalTx = objResult =>
            !!(objResult.tx.payload.outs.length && objResult.tx.payload.outs[0].intTx.length);
    ...
    stripAddressPrefix(strAddr) {
        ...
        return strAddr.substring(0, 2) === prefix ? strAddr.substring(prefix.length) : strAddr;
    }

    async queryRpcMethod(strName, objParams) {
        const res = await this._client.request(strName, objParams);
    ...
    calculateTxFee(tx, bWithChange = true, bOneSignature = true) {
        ...
        return bOneSignature
            ? this._nFeePerInputNoSign * nInputsCount +
                  this._nFeePerInputOutput * (nOutputsCount + (bWithChange ? 1 : 0))
